  ...
  Future<sqflite.Database> open(
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        ...
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `comments` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `text` TEXT NOT NULL, `postId` INTEGER NOT NULL)');
...
class _$PostDao extends PostDao {
  ...
  Future<void> insertPost(Post post) async {
    await _postInsertionAdapter.insert(post, OnConflictStrategy.replace);
  }
}
...
class _$CommentDao extends CommentDao {
  ...
  Future<List<Comment>> findCommentsForPost(int postId) async {
    return _queryAdapter.queryList('SELECT * FROM comments WHERE postId = ?1',
  ...
  Future<void> insertComment(Comment comment) async {
    await _commentInsertionAdapter.insert(comment, OnConflictStrategy.replace);
  }
}
